...
def recommend_product_based_on_user(user_id):
    ...
    similar_users = model.get_neighbors(
        trainset.to_inner_uid(user_id), k=num_neighbors)
...
def recommend_product_based_on_product(picked_product):
    ...
    cosine_sim_keywords = linear_kernel(
        tfidf_matrix_keywords, tfidf_matrix).flatten()
    ...
    keyword_matching_indices = sorted(
        keyword_matching_indices, key=lambda x: x[1], reverse=True)
...
def signup():
    ...
    if request.form.get("product_id"):
        ...
        session["userid"] = df.loc[df["username"]
